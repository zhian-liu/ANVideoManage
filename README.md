...
5. **录像保存**：进入「系统设置」→「基础设置」，选择「永久保存」或「按天自动清理」，自定义保留 1–3650 天后点击「保存设置」。默认永久保存；切回永久保存可关闭自动清理，配置在重启后仍有效。
6. **选择存储目录**：点击录像或抓拍路径左侧的文件夹图标，在弹窗中选择后端主机的磁盘、进入文件夹，点击「选择此文件夹」回填路径，再点击「保存设置」。也支持输入路径、返回上一级；取消选择不修改原路径。
...
- **自动清理**：后端启动、保存设置后以及运行期间每小时检查一次。按录像结束时间清理超过保留天数的已完成录像，同时删除磁盘文件和回放索引；历史录像也适用，抓拍和正在录制的分片不受影响。文件不存在时移除失效索引，文件被占用或无权限删除时保留索引并在下一轮重试。清理后的录像无法恢复。
...
- **录像没有生成**：确认设备「启用录像」开启；ZLMediaKit 的 `protocol.enable_mp4` 保持为 `0`，由后端按设备逐路控制。等待一个分片时长后刷新回放页。
